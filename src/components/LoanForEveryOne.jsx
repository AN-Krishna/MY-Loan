...
const LoanForEveryone = () => {
  ...
  return (
    ...
    <div className='container mx-auto px-4 py-8 bg-blue-300 mt-4'>
      <h1 className='text-4xl text-blue-500 font-bold text-center font-serif py-8'>Loan for Everyone</h1>
      ...
      <div className='flex justify-center flex-wrap gap-4 py-8'>
        ...
        <button
          className={`w-60 px-6 py-3 rounded-lg flex items-center justify-center ${
            activeTab === 'College Students' ? 'bg-blue-500 text-white font-semibold' : 'bg-gray-200 text-blue-500'
          }`}
        ...
        <button
          className={`w-60 px-6 py-3 rounded-lg flex items-center justify-center ${
            activeTab === 'Working Professionals' ? 'bg-blue-500 text-white font-semibold' : 'bg-gray-200 text-blue-500'
          }`}
        ...
        <button
          className={`w-60 px-6 py-3 rounded-lg flex items-center justify-center ${
            activeTab === 'Self Employed' ? 'bg-blue-500 text-white font-semibold' : 'bg-gray-200 text-blue-500'
          }`}
      ...
      {/* Content Section */}
      <div className='bg-white grid grid-cols-1 lg:grid-cols-2 gap-6 px-4 lg:px-16 py-8 rounded-xl mx-auto max-w-screen-lg'>
        <div className='flex flex-col items-start justify-center p-6 rounded-lg'>
          <h2 className='text-blue-500 text-3xl font-bold mb-4'>{content[activeTab].title}</h2>
          <p className='text-blue-600'>{content[activeTab].description}</p>
        </div>
        <div className='flex justify-center'>
          <img
            src={content[activeTab].imgSrc}
            alt={activeTab}
            className='rounded-lg w-full h-64 object-cover max-w-xs'
          />
        </div>
      </div>
    </div>
